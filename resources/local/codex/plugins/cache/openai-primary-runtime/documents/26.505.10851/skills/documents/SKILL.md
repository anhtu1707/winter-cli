...
If creating a new first-page header, cover, or title block, also read `references/header_templates.md` and choose one header pattern before drafting.

Then resolve the preset into a token map and apply the tokens consistently:

1. Set page, margin, type scale, paragraph rhythm, heading, list, table, callout, header, footer, and color tokens before drafting.
